...
class CodeServerProxifier extends EventEmitter {
  constructor() {
    super();
    // key为ip,value为目标机器连接隧道，方便动态代理到目标机器
    this.codeServers = new Map();
  }

  connect(connectOpts) {
    if (this.codeServers.get(connectOpts.host)) {
      this.emit('ready');
    } else {
      let codeServerProxy = new CodeServerProxy();
      codeServerProxy.connect(connectOpts)
        .on('ready', () => {
          this.codeServers.set(connectOpts.host, codeServerProxy);
          this.emit('ready');
        });
    }
  }
...
class CodeServerProxy extends SshClient {
  ...
  connect(connectOpts) {
    ...
    client.connect(connectOpts).on('ready', () => {
      client.conn.forwardOut('127.0.0.1', 8080, '127.0.0.1', codeServerPort, (err, stream) => {
        if (err) {
          throw err;
        }
        this.stream = stream;
