 ...
 *
 *     sudo systemctl enable --now webshell-code-server@$USER
 *     sudo systemctl start webshell-code-server@$USER
 *     sudo systemctl stop webshell-code-server@$USER
...
const COMMANDS = {
   ...
   */
  installPackage: 'curl -fsSL https://raw.githubusercontent.com/alanhg/express-demo/master/lib/code-server/code-server-install.sh | sh',

  unInstallPackage: 'rm -rf ~/.webshell/code-server/.local/share/code-server ~/.webshell/code-server/.config/code-server',

  // 0表示已安装
  checkInstall: `if which /usr/bin/code-server >/dev/null; then
    echo installed
else
    echo uninstalled
fi`,
...
class CodeServerStarter {
  ...
  async start() {
    ...
    if (codeServerServiceStatus === 'unknown' || codeServerServiceStatus === 'inactive' || codeServerServiceStatus === 'failed') {
      // 执行启动
      await this.sshClient.execCommandPromisefy(COMMANDS.startService);
      console.log(chalk.blue(`code server服务状态为${codeServerServiceStatus}，启动成功`));
