...
const COMMANDS = {
   ...
   * 删除服务及所有文件
   */
  uninstallService: 'sudo systemctl stop webshell-code-server@$USER && rm -f /usr/lib/systemd/system/webshell-code-server@.service && rm -rf ~/.webshell/code-server',
...
class CodeServerStarter {
  ...
  async start() {
    ...
    if (codeServerServiceStatus === 'active') {
      console.log(chalk.blue('code server服务已启动，创建代理服务'));
