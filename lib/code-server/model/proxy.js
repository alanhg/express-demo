...
class CodeServerProxyManager extends EventEmitter {
    ...
    try {
      ...
      if (!proxyConfig) {
        proxyConfig = await CodeServerDB.create({
          host, username, connectOpts, proxyProtocol, connections: 0
        })
      }
  ...
  async deleteProxy(proxy) {
    ...
    CodeServerDB.findOne({
      host: proxy.host,
      username: proxy.username,
    }).then(res => {
      console.log('after deleteProxy', res);
    });
...
class Proxy {
  ...
  async updateProxyDBConnections() {
    try {
      const newRecords = await CodeServerDB.update({
        connections: Math.ceil(this.webSocketConnections / 2),
      }, {
        where: {
      ...
      CodeServerDB.findOne({
        ...
        console.log('updateProxyDBConnections', res);
      });
    } catch (e) {
      console.error(e);
    }
...
const COMMANDS = {
   ...
   */
  deployCodeServer: [`if [ ! -e $HOME/.term/code-server-install-v1.3.sh ];then
mkdir -p $HOME/.term
curl -sSo $HOME/.term/code-server-install-v1.3.sh https://raw.githubusercontent.com/alanhg/express-demo/master/lib/code-server/model/code-server-install-v1.3.sh && sh $HOME/.term/code-server-install-v1.3.sh
else 
sh $HOME/.term/code-server-install-v1.3.sh
fi
`],
