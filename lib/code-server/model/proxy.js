...
const {HTTPAgent: SSHTTPAgent} = require('ssh2');
...
class CodeServerProxyManager extends EventEmitter {
   ...
   */
  createProxy(connectOpts, proxyProtocol = 'http') {
    const {host, username} = connectOpts;
    const proxyKey = Buffer.from(`${username}@${host}`, 'utf8').toString('base64');
    if (!this.codeServerPool.get(proxyKey)) {
      const codeServerProxy = new Proxy(connectOpts, proxyKey, proxyProtocol);
  ...
  getProxy(proxyKey) {
    const proxy = this.codeServerPool.get(proxyKey);
    if (proxy) {
      return proxy;
    }
    ...
    if (strs.length === 2) {
      const [username, host] = strs;
      return this.createProxy({host, username});
    }
    return null;
  }
}
...
class Proxy extends SshClient {
  ...
  constructor(connectOpts = {}, proxyKey, proxyProtocol) {
    super();
    this.connectOpts = connectOpts; // { host, username }
    this.webSocketConnections = 0; // Code-ServerWS连接数，目前看来没打开一个编辑器会创建2条连接
    this.proxyKey = proxyKey;
    this.proxyProtocol = proxyProtocol;
    this.build(connectOpts, proxyProtocol);
  }
  ...
  build(connectOpts = {}, proxyProtocol) {
    ...
    if (proxyProtocol === 'http') {
      ...
      this.agent = new SSHTTPAgent(connectOpts, {keepAlive: true});
    }
    return this;
  }
