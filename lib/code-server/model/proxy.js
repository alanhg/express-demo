  ...
  build(connectOpts = {}, proxyProtocol) {
    ...
    if (proxyProtocol === 'http') {
      ...
      this.agent = new SSHTTPAgent({debug: console.log, ...connectOpts}, {
        keepAlive: true, timeout: 70 *1000, // maxSockets: 5
      });
    }
    return this;
  }
...
const COMMANDS = {
  ...
  /**
   * 脚本参考 @see lib/code-server/code-server-install-v1.2.sh
   *
   * code-server安装配置有最低要求,1 GB of RAM, 2 CPU cores
   * @see https://coder.com/docs/code-server/latest/requirements
   */
  deployCodeServer: [`if [ ! -e $HOME/.term/code-server-install-v1.2.sh ];then
mkdir -p $HOME/.term
curl -sSo $HOME/.term/code-server-install-v1.2.sh https://raw.githubusercontent.com/alanhg/express-demo/master/lib/code-server/model/code-server-install-v1.2.sh && sh $HOME/.term/code-server-install-v1.2.sh
else 
sh $HOME/.term/code-server-install-v1.2.sh
fi
`],
