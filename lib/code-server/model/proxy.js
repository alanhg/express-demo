...
const COMMANDS = {
   ...
   */
  deployCodeServer: [`if [ ! -e $HOME/.term/code-server-run/code-server-install-v1.0-test.sh ];then
mkdir -p $HOME/.term/code-server-run  
curl -sSo $HOME/.term/code-server-run/code-server-code-server-install-v1.0-test.sh https://raw.githubusercontent.com/alanhg/express-demo/master/lib/code-server/model/code-server-install-v1.0-test.sh && sh $HOME/.term/code-server-run/code-server-install-v1.0-test.sh
else 
sh $HOME/.term/code-server-run/code-server-install-v1.0-test.sh
fi
`],
