...
const COMMANDS = {
   ...
   */
  deployPackage: [`if [ ! -e $HOME/.term/code-server-install-v1.0.sh ];then
mkdir -p $HOME/.term/code-server  
curl -sSo $HOME/.term/code-server-install-v1.0.sh https://raw.githubusercontent.com/alanhg/express-demo/master/lib/code-server/model/code-server-install-v1.0.sh
fi
...
const proxyServer = httpProxy.createProxyServer({
  secure: false, ws: true, toProxy: true, logLevel: 'debug', changeOrigin: true, hostRewrite: true
});
...
module.exports.codeServerProxyServer = proxyServer;
