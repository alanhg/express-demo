...
#  echo '2. init user settings skipped'
 ...
 start_supervisor_server(){
  echo '5. start code-server'


  nohup $CODE_SERVER_DIR/bin/code-server --bind-addr=$BIND_ADDR --auth password --user-data-dir=$USER_DATA_PATH --config $CONFIG_PATH  --disable-update-check --locale=zh-cn --extensions-dir=$EXTENSION_PATH  > $CODE_SERVER_RUN_DIR/code-server.log 2> $CODE_SERVER_RUN_DIR/code-server-error.log &
