...
class SshShellClient extends SshClient {
  ...
  connect(connectOpts) {
    const codeServerProxy = new CodeServerProxy();
      ...
      if (options.type === 'codeserver') {
        ...
        if (codeServerServiceStatus.match(/^active/)) {
          ...
          if (this.codeServerProxy) {
            ...
            codeServerProxy.connect(this.config);
            codeServerProxy.on('ready', () => {
              codeServerProxy.proxyCodeWeb().then(() => {
                this.codeServerProxy = codeServerProxy;
                this.send('codeserver', {
                  message: 'run ok',
                })
              })
