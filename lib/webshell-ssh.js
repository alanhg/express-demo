...
const {commands, codeServerProxifier} = require("./code-server/code-server-proxy");
    ...
    this.ws.on('message', async (msg) => {
      ...
      if (options.type === 'codeserver') {
        ...
        if (codeServerServiceStatus.match(/^unknown/)) {
          const codeServerInstallStatus = await this.execCommandPromisefy(`which code-server`);
        ...
        } else if (codeServerServiceStatus.match(/^(inactive|failed)/)) {
          // 执行启动
        ...
        if (codeServerServiceStatus.match(/^active/)) {
          ...
          let id = codeServerProxifier.connect(this.config);
